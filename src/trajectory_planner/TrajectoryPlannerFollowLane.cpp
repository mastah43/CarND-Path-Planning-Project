...
TrajectoryPlannerFollowLane::planTrajectory(EgoVehicleState &egoState, const SensorFusionResult &sensorFusion,
    ...
    std::cout << "ego coord s: " << egoState.getFrenet().s << " m" << std::endl;
    egoState.getFrenet().cout("ego pos frenet: ");
    ...
    egoStateNew.getFrenet().cout("ego pos frenet new: ");
    std::cout << std::endl;
    std::cout << "ego pos xy new: " << egoStateNew.getXY().x << ";" << egoStateNew.getXY().y << std::endl;
    ...
    */
    ...
    trajectoryAnchor.transform(-egoStateNew.getX(), -egoStateNew.getY(), -egoStateNew.getYaw());
    ...
    for (int i=0; i<waypointsNewCount; i++) {
        ...
        xy.transform(egoStateNew.getX(), egoStateNew.getY(), egoStateNew.getYaw());
...
EgoVehicleState TrajectoryPlannerFollowLane::applyPreviousTrajectory(EgoVehicleState &egoState,
    ...
    if (trajectoryPrevious.size() < 2) {
        ...
        trajectory.append(egoState.getXY().movedYawXY(egoState.getYaw(), -1));
        ...
        egoStateNewTrajectory.setPos(xyLast, getMap().getFrenet(xyLast, egoState.getYaw()));
